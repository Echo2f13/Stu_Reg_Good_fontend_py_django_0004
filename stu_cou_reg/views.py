...
#student reg_view:
def student_course_view_activity(request):
    if request.method == 'POST':
        ...
        if 'Approved' in request.POST:
            print('PASS 2')
            return render(request, 'student_profile_active.html', {'user': User.objects.filter( id= user_id), 'std' : StudentData.objects.filter( user_id = user_id ), 'cou' : CourseData.objects.all(), 'reg' : RegisteredData.objects.filter( student_id_id = stu_data.student_id ) })
        elif 'Pending' in request.POST:
            print('PASS 3')
            return render(request, 'student_profile_pending.html', {'user': User.objects.filter( id= user_id), 'std' : StudentData.objects.filter( user_id = user_id ), 'cou' : CourseData.objects.all(), 'reg' : RegisteredData.objects.filter( student_id_id = stu_data.student_id ) })
        elif 'Rejected' in request.POST:
            print('PASS 4')
            return render(request, 'student_profile_rejected.html', {'user': User.objects.filter( id= user_id), 'std' : StudentData.objects.filter( user_id = user_id ), 'cou' : CourseData.objects.all(), 'reg' : RegisteredData.objects.filter( student_id_id = stu_data.student_id ) })
...
def edit_student_profile(request):
    if request.method == 'POST':
        ...
        std_data = StudentData.objects.filter( user_id = stu_user_id ).first()
        if 'edit' in request.POST:
            print('PASS 2')
            print(user)
            print(user.first_name)
            print(user.last_name)
            image_default = std_data.student_image
            print(image_default)
            user.first_name = request.POST["student_first_name_pre_2"]
            user.last_name = request.POST["student_last_name_pre_2"]
            std_data.student_image = request.FILES.get("image")
            user.save();
            if std_data.student_image != None :
                std_data.save();
                print("new",std_data.student_image)
            print("new",user.first_name)
            print("new",user.last_name)
            return render(request, 'std_profile.html', {'user': User.objects.filter( id = stu_user_id ), 'std' : StudentData.objects.filter( user_id = stu_user_id ), 'cou' : CourseData.objects.all(), 'reg' : RegisteredData.objects.filter( student_id_id = stu_data_pre_2.student_id ), 'image_default' : image_default })
...
def course_reg_form(request):
    ...
    if request.method == 'POST':
        ...
        print(user)
        print(user.id)
        print(reg_id,'passesd not passed yet')
        if 'Register' in request.POST:
            print(reg_id,'passesd not passed yet 2')
            exists = RegisteredData.objects.filter( student_id_id = stu_data.student_id, course_id_id = reg_id ).first()
            if not exists :
                print('good going')
                reg_data = RegisteredData.objects.create( student_id_id = stu_data.student_id, course_id_id = reg_id )
                now = datetime.now()
                print("now =", now)
                # dd/mm/YY H:M:S
                dt_string = now.strftime("%d/%m/%Y %H:%M ")
                print("date and time =", dt_string)
                reg_data.date_apply = dt_string
                stu_data.save();
                reg_data.course_status = 0
                stu_data.save();
                user.save();
                reg_data.save();
                print(reg_id,'passesd')
...
def std_reg_status_approval(request, id, course_id):
    ...
    dt_string = now.strftime("%d/%m/%Y %H:%M")
    print("date and time =", dt_string)
    reg_data.date_approve = dt_string
    reg_data.save();
    stu_data.save();
    user.save();
    print("got approved")
